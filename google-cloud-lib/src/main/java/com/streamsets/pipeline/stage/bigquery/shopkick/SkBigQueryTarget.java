...
public class SkBigQueryTarget extends BigQueryTarget {
	...
	protected void handleTableNotFound(Record record, String datasetName, String tableName,
		...
		if (result == null || !result.result) {
			...
			LOG.info("Auto create failed for table {}.{}. Message: {}", datasetName, tableName, result.message);
			super.handleTableNotFound(record, datasetName, tableName, null);
		}
	}

	@Override
	protected void handleInsertErrors(TableId tableId, ELVars elVars, Map<Long, Record> requestIndexToRecords,
	...
	private void insertBatchAgain(TableId tableId, ELVars elVars, Map<Long, Record> requestIndexToRecords,
			List<Record> retry, List<ErrorRecord> stopped) {
	...
	private void addMissingColumnsInBigQuery(TableId tableId, List<Record> retry, List<ErrorRecord> missingCols) {
	...
	private void insertAllWithRetries(Map<Long, Record> requestIndexToRecords, ELVars elVars, TableId tableId,
			InsertAllRequest request, int sleepTimeSec, int elapsedSec) {

		int elapsedMin = elapsedSec / 60;
		if (elapsedMin > maxWaitTimeForInsertMins) {
			LOG.warn("Cannot push message through retries. Elapsed: {} Secs. Trying once more", elapsedSec);
			...
			try {
				...
				if (response.hasErrors()) {
					...
					if (missingCols.size() > 0) {
						...
						super.handleInsertErrors(tableId, elVars, requestIndexToRecords, response);
						return;
					}
				} else {
	...
	private void addToInsertRequest(ELVars elVars, List<Record> retry,
		...
		retry.forEach(record -> {
			try {
				...
				LOG.error("Error when converting record {} to row, Reason : {} ", record.getHeader().getSourceId(),
						e.getMessage());
	...
	private void bucketizeErrors(Map<Long, Record> requestIndexToRecords, InsertAllResponse response,
		...
		response.getInsertErrors().forEach((requestIdx, errors) -> {
			...
			LOG.info("<Auto_add_column> Handling Error when inserting record {}, Reasons : {}, Messages : {}",
			...
			} else if (REASON_INVALID.equalsIgnoreCase(reasons) && NO_SUCH_FIELD.equalsIgnoreCase(messages)) {
	...
	private Result addMissingColumnsInBigQuery(TableId tableId, Record record) {
		...
		if (!result.fields.isEmpty()) {
			...
			TableInfo tableInfo = table.toBuilder().setDefinition(StandardTableDefinition.of(schema)).build();
			Table update = bigQuery.update(tableInfo);
			LOG.info("Table updated with additional fields: " + update);

			boolean updateDone = isTableUpdated(tableId, result);
	...
	private boolean isTableUpdated(TableId tableId, Result result) {
		int retryCount = 0;
		boolean updateDone = checkTableHasNewFields(tableId, result.fields);
		while (!updateDone && retryCount < ADD_COLS_RETRIES) {
			try {
				LOG.debug("Sleeping for {} seconds for checking whether table {} updated", RETRY_SLEEP_TIME_MS,
						tableId);
				TimeUnit.MILLISECONDS.sleep(RETRY_SLEEP_TIME_MS);
			} catch (InterruptedException e) {
				LOG.debug("Interrupted", e);
			}
			updateDone = checkTableHasNewFields(tableId, result.fields);
			retryCount++;
		}
		return updateDone;
	}

	private boolean checkTableHasNewFields(TableId tableId, List<com.google.cloud.bigquery.Field> allFields) {
		Table table = bigQuery.getTable(tableId, TableOption.fields(TableField.SCHEMA));
		LOG.info("Table Status after update: TableId:{}, Table: {}", tableId, table);
		List<com.google.cloud.bigquery.Field> tableFields = table.getDefinition().getSchema().getFields();
		if (tableFields.size() != allFields.size()) {
			LOG.debug("Fields Mismatch : Expected: {}, Actual: {}", allFields, tableFields);
			return false;
		}
		return true;
	}
	...
	private void createTable(TableId tableId, List<com.google.cloud.bigquery.Field> fields) {
		...
		String tableName = tableId.getTable();
		if (isPartitioned(tableName)) {
			TableDefinition tableDefinition = StandardTableDefinition.of(schema).toBuilder()
					.setTimePartitioning(TimePartitioning.of(com.google.cloud.bigquery.TimePartitioning.Type.DAY))
					.build();
			String onlyTableName = extractTableName(tableName);
			tableInfo = TableInfo.newBuilder(TableId.of(tableId.getDataset(), onlyTableName), tableDefinition).build();
		} else {
	...
	private com.google.cloud.bigquery.Field getFieldForList(Field field, String fieldPath) {
		...
		if (bqType != null) {
			return com.google.cloud.bigquery.Field.of(fieldPath, bqType).toBuilder().setMode(Mode.REPEATED).build();
		} else {
			return null;
		}
	}

	private static Map<Field.Type, com.google.cloud.bigquery.Field.Type> DATA_TYPE_MAP = new HashMap<>();
	static {
